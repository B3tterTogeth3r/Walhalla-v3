...
import de.b3ttertogeth3r.walhalla.old.models.Account;
...
public class Firestore {
            ...
            if (person.getId() == null) {
                ...
                FIRESTORE.collection("Person").document(person.getId()).set(person.toMap())
                        .addOnCompleteListener(task -> {
                            ...
                            // throw new NullPointerException("I want to crash to get an error");
                        })
                        .addOnFailureListener(e -> {
    ...
    public static void findUserCharge (MyCompleteListener<String> listener) {
        ...
        FIRESTORE.collection("Current")
                .whereArrayContains("UID", Authentication.getUser().getUid())
                .get()
                .addOnCompleteListener(task -> {
    ...
    public static void uploadCharge (@NonNull Semester chosenSemester,
                                     @NonNull de.b3ttertogeth3r.walhalla.old.models.Charge charge
            , @NonNull Charge kind) {
    ...
    public static void updateFCM (String string) {
        if (Authentication.isSignIn()) {
            FIRESTORE.collection("Person").document(Authentication.getUser().getUid())
                    .update("fcm_token", string).addOnCompleteListener(task -> {
                        if(task.getException() != null) {
                            MyLog.e(TAG, "updating fcm-token didn't work", task.getException());
                        }
                    });
        }
    }
