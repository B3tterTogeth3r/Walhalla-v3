...
import de.b3ttertogeth3r.walhalla.old.dialog.EditChargeDialog;
...
public class ChargenFragment extends CustomFragment {
            ...
            public void onSuccess (QuerySnapshot querySnapshot) {
                ...
                for (DocumentSnapshot snap : querySnapshot) {
                    try {
                        ...
                        if (c != null) {
                            c.setId(snap.getId());
                            de.b3ttertogeth3r.walhalla.old.enums.Charge charge =
                                    de.b3ttertogeth3r.walhalla.old.enums.Charge.find(c.getId());
    ...
    private AlertDialog getEditDialog () {
        ...
        builder.setTitle(R.string.dialog_charge_choose)
                ...
                .setItems(active_board_members, (dialog, which) -> {
                    ...
                    de.b3ttertogeth3r.walhalla.old.enums.Charge selected =
                            de.b3ttertogeth3r.walhalla.old.enums.Charge.find(active_board_members[which]);
                    try {
                        switch (selected) {
                            case X:
                                charge = board.get(0);
    ...
    private void updateList () {
        ...
        for (Charge b : board) {
            try {
                de.b3ttertogeth3r.walhalla.old.enums.Charge selected =
                        de.b3ttertogeth3r.walhalla.old.enums.Charge.find(b.getId());
                if (selected != NONE) {
