package de.b3ttertogeth3r.walhalla.fragments.home;
...
public class Fragment extends CustomFragment implements View.OnClickListener {
    ...
    public void createView (@NonNull View view,
        ...
        if (Firebase.Authentication.isSignIn()) { // user is signed in:
            // region balance
        ...
        if (!Firebase.Authentication.isSignIn()) { // No user is signed in
            // Add icon
    ...
    public void onClick (@NonNull View v) {
        switch (v.getId()) {
            case R.id.program:
                ...
                Firebase.Analytics.screenChange(v.getId(), getString(R.string.menu_program));
                getParentFragmentManager().beginTransaction().replace(R.id.fragment_container,
                        new de.b3ttertogeth3r.walhalla.fragments.program.Fragment()).commit();
                break;
            case R.id.greeting:
                Log.d(TAG, "onClick: greeting");
                Firebase.Analytics.screenChange(v.getId(), getString(R.string.menu_greeting));
                getParentFragmentManager().beginTransaction().replace(R.id.fragment_container,
                        new de.b3ttertogeth3r.walhalla.fragments.greeting.Fragment()).commit();
                break;
            case R.id.chargen:
                Log.d(TAG, "onClick: chargen");
                Firebase.Analytics.screenChange(v.getId(), getString(R.string.menu_chargen));
                getParentFragmentManager().beginTransaction().replace(R.id.fragment_container,
                        new de.b3ttertogeth3r.walhalla.fragments.chargen.Fragment()).commit();
                break;
            case R.id.news:
                Log.d(TAG, "onClick: news");
                Firebase.Analytics.screenChange(v.getId(), getString(R.string.menu_messages));
                getParentFragmentManager().beginTransaction().replace(R.id.fragment_container,
                        new de.b3ttertogeth3r.walhalla.fragments.news.Fragment()).commit();
                break;
            case R.id.row2first:
                if (Authentication.isSignIn()) {
                    Log.d(TAG, "onClick: balance");
                    Firebase.Analytics.screenChange(v.getId(), getString(R.string.menu_balance));
                    getParentFragmentManager().beginTransaction().replace(R.id.fragment_container,
                            new de.b3ttertogeth3r.walhalla.fragments.balance.Fragment()).commitNowAllowingStateLoss();
                } else {
                    Log.d(TAG, "onClick: rooms");
                    Firebase.Analytics.screenChange(v.getId(), getString(R.string.menu_rooms));
                    getParentFragmentManager().beginTransaction().replace(R.id.fragment_container,
            ...
            case R.id.login:
                if(Authentication.isSignIn()){
                    ...
                    Firebase.Analytics.screenChange(v.getId(), getString(R.string.menu_home));
                    getParentFragmentManager().beginTransaction().replace(R.id.fragment_container,
                            new de.b3ttertogeth3r.walhalla.fragments.home.Fragment()).commit();
                } else {
                    Log.d(TAG, "onClick: login");
                    Firebase.Analytics.screenChange(v.getId(), getString(R.string.menu_login));
                    getParentFragmentManager().beginTransaction().replace(R.id.fragment_container,
